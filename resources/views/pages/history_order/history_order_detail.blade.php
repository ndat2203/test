        ...
        <div class="container mb-5">
            ...
            @foreach ($all_order as $order => $data_order)
                        <div class="order-detail-card p-4">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <p><strong>Họ tên:</strong> {{ $data_order->first()->customer_name }} </p>
                                    <p><strong>Địa chỉ:</strong> {{ $data_order->first()->shipping_address }} </p>
                                </div>
                                <div class="col-md-6">
                                    <p><strong>SĐT:</strong> {{ $data_order->first()->shipping_phone }} </p>
                                    <p><strong>Email:</strong> {{ $data_order->first()->shipping_email }} </p>
                                </div>
                            </div>
                            ...
                            <div class="order-meta mb-4">
                                <p><strong>Ngày đặt: {{ date('m/d/Y', strtotime($data_order->first()->order_date)) }}</strong> </p>
                                <p><strong>Trạng thái:</strong>
                                    <span
                                        class="{{ $data_order->first()->order_status == 'Đã giao' ? 'text-success' : 'text-warning' }}">
                                        {{ $data_order->first()->order_status }}
                                    </span>
                                </p>
                                <p><strong>Phương thức thanh toán:</strong>
                                    {{ $data_order->first()->payment_method ?? 'Không xác định' }} </p>
                            </div>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach($data_order as $product)
                                            <tr>
                                                <td>
                                                    <img src="{{ asset('public/upload/product/' . $product->product_image) }}"
                                                        width="60" height="60"
                            ...
                            @php
                                ...
                                $shipping_fee = $data_order->first()->fee_ship ?? 0;
                                $discount_amount = $data_order->first()->product_counpon_percent ?? 0;
                            ...
                            <div class="text-right mt-3">
                                <p><strong>Tạm tính:</strong> {{ number_format($subtotal, 0, ',', '.') }} VNĐ</p>
                                        ...
                                        <p><strong>Phí vận chuyển:</strong> {{ number_format($data_order->first()->fee_ship ?? 0, 0, ',', '.') }}  VNĐ</p>
                                        <p><strong>Giảm giá: </strong> {{ number_format($data_order->first()->product_counpon_percent ?? 0, 0, ',', '.') }}  VNĐ</p>
                                        <hr style="border-top: 1px dashed #ccc;">
                                        <p style="font-size: 16px; font-weight: bold; color: #e74c3c;">Tổng thanh toán: {{ number_format($total_payment, 0, ',', '.') }}  VNĐ</p>
                            </div>
                        </div>
            @endforeach
        </div>
