...
<div class="features_items"><!--features_items-->
            @foreach($brand_name  as $key => $value)
                @if($value->brand_status == 1)
                    <h2 style="margin-top:5px;" class="title text-center">Thương hiệu {{ $value->brand_name }}</h2>
                @endif
            @endforeach
            ...
            @foreach($brand_by_id as $key => $product)
                @if($product->brand_status == 1 && $product->product_status == 1)
            ...
            <div class="col-sm-12" style="display:flex;justify-content:end;">
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-end">
                        {{ $brand_by_id->links('vendor.pagination.bootstrap-4') }}
                    </ul>
                </nav>
            </div>
</div><!--features_items-->
