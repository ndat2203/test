...
<header id="header"><!--header-->
				...
				<div class="col-sm-3">
					<div class="left-sidebar">
						<div class="brands_products"><!--brands_products-->
                            <h2 style="margin-top:5px;" class="title text-center">Bài viết mới nhất</h2>
							<div class="brands-name" style="border: none; border-radius: 6px; padding: 10px;">
    ...
    @foreach ($latest_posts as $post)
        <div style="
            ...
            padding-bottom: 10px;
        ">
            <img src="{{ URL::to('public/upload/post/'.$post->post_image) }}" alt="{{ $post->post_slug }}" style="
                width: 90px;
                height: 65px;
                object-fit: cover;
                border-radius: 6px;
            ...
            <div style="flex: 1;">
                <h4 style="margin: 0 0 5px 0; font-size: 15px; line-height: 1.4;">
                    <a href="{{ URL::to('/bai-viet/'.$post->post_slug) }}" style="text-decoration: none; color: #333;">
                    {!! \Illuminate\Support\Str::words(html_entity_decode($post->post_title), 10, '...') !!}
                    </a>
                </h4>
                <p style="font-size: 13px; color: #666; margin: 0 0 8px 0; line-height: 1.5;">
                {!! \Illuminate\Support\Str::words(html_entity_decode($post->post_desc), 10, '...') !!}
                </p>
                <a href="{{ URL::to('/bai-viet/'.$post->post_slug) }}" style="
                    font-size: 12px;
                    color: #fff;
    ...
    @endforeach
				...
				<div class="col-sm-9 padding-right">
                    <div class="features_items"><!--features_items-->
                        @if($cate_post_name && $cate_post_name->cate_post_status == 1)
                            <h2 style="margin-top:5px;" class="title text-center">Danh mục tin tức {{  $cate_post_name->cate_post_name }}</h2>
                        @endif
                        <div class="product-image-wrapper">
                            @foreach($posts as $key => $val)
                                <div class="single-products" style="margin: 15px 0; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background-color: #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                                    <div style="display: flex; align-items: flex-start;">
                                        <img
                                            src="{{ URL::to('public/upload/post/'.$val->post_image) }}"
                                            alt="{{ $val->post_slug }}"
                                            style="width: 30%; max-width: 150px; height: auto; border-radius: 5px; margin-right: 15px;"
                                        />
                                        <div style="flex: 1;">
                                            <h4 style="margin-top: 0; font-size: 18px; color: #333;">{{ $val->post_title }}</h4>
                                            <p style="color: #555; font-size: 14px; line-height: 1.5;">{!! $val->post_desc !!}</p>
                                            <div style="text-align: right; margin-top: 10px;">
                                                <a href="{{ url('/bai-viet/'.$val->post_slug) }}"
                                                ...
                                                onmouseout="this.style.backgroundColor='#f7941d'; this.style.transform='scale(1)'"
                                                >
                                                    Xem bài viết
                                                </a>
                                            </div>
                            ...
                            @endforeach
                                ...
                                <ul class="pagination pagination-sm justify-content-center" style="float:right;">
